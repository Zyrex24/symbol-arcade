...
type GameType = "menu" | "guess" | "tictactoe" | "snake" | "rps";

export default function App() {
  const [currentGame, setCurrentGame] = useState<GameType>("menu");

  const Header = (
    <header className="w-full z-10 sticky top-0 bg-gradient-to-r from-indigo-700/80 via-purple-700/80 to-fuchsia-700/80 shadow-lg backdrop-blur flex items-center justify-between px-6 py-3 rounded-b-3xl">
      <div className="flex items-center gap-3">
        <span className="text-3xl font-black text-white tracking-tight drop-shadow">
          WASM Arcade
        </span>
        <span className="ml-2 px-2 py-1 rounded bg-white/20 text-white text-xs font-mono tracking-widest">
          v1.0
        </span>
      </div>
      <div className="flex items-center gap-4">
        ...
        <button
          className="rounded-full bg-white/20 hover:bg-white/40 p-2 transition"
  ...
  if (currentGame !== "menu") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-700 flex flex-col items-center justify-center p-4">
        {Header}
